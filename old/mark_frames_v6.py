...
def stable_marker_detection(gray, center_x, runs=200, jitter_px=2, std_threshold=5):
    top_y_list = []
    bottom_y_list = []

    for _ in range(runs):
        jittered_center_x = center_x + np.random.randint(-jitter_px, jitter_px + 1)
        top_y, bottom_y = detect_markers(
            gray, jittered_center_x,
            scan_width=10
            ,threshold_ratio=0.3, min_valid_distance=50,
            max_band_thickness=5
    ...
    if len(top_y_list) >= runs // 2:
        top_y_med = int(median(top_y_list))
        bottom_y_med = int(median(bottom_y_list))
        std_top = round(stdev(top_y_list), 2) if len(top_y_list) > 1 else 0
        std_bottom = round(stdev(bottom_y_list), 2) if len(bottom_y_list) > 1 else 0

        # Reject frame if std exceeds threshold
        if std_top > std_threshold or std_bottom > std_threshold:
            return None, None, std_top, std_bottom, True  # unstable
        else:
            return top_y_med, bottom_y_med, std_top, std_bottom, False  # good
...
def process_images():
    ...
    if total_frames <= (skip_start_frames + skip_end_frames):
        print(f"❌ Not enough frames. Total={total_frames}, trying to skip {skip_start_frames} + {skip_end_frames}")
        return

    # Apply skipping
    frames = frames[skip_start_frames : total_frames - skip_end_frames]
    ...
    for i, frame in enumerate(frames):
        ...
        strip_width = 10
        x1 = max(center_x - strip_width, 0)
        x2 = min(center_x + strip_width, gray.shape[1])
        rebar_strip = gray[:, x1:x2]

        top_y, bottom_y, std_top, std_bottom, is_unstable = stable_marker_detection(rebar_strip, strip_width)


        if top_y is None or bottom_y is None:
            print(f"⚠ Skipping frame {frame}: markers not found at all.")
            continue
        std_avg = (std_top + std_bottom) / 2
        confidence = max(0.0, 1.0 - std_avg / max_std)
        confidence = round(confidence, 3)
        if is_unstable:
            print(f"⚠ Low-confidence frame {frame} (std top={std_top}, bottom={std_bottom}, confidence={confidence}) — keeping anyway.")
        ...
        bar5_x_start = img.shape[1] - 5 - bar_margin_right - 45
        cv2.line(img, (bar5_x_start, bar_y), (bar5_x_start + 5, bar_y), (255, 255, 255), bar_thickness)
        cv2.putText(img, "5 px", (bar5_x_start - 5, bar_y - 10), font, font_scale, (255, 255, 255), font_thickness)

        bar50_x_start = img.shape[1] - 50 - bar_margin_right
        cv2.line(img, (bar50_x_start, bar_y - 30), (bar50_x_start + 50, bar_y - 30), (255, 255, 255), bar_thickness)
