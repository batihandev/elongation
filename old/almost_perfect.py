...
def filter_consistent_patterns(patterns, pattern_capture, position):
    consistent = []
    for p in tqdm(patterns, desc=f"Filtering {position} patterns"):
        matched_all = True
        for gray, center_x, *_ in pattern_capture:
            ...
            strip = gray[:, x1:x2]
            search_range = (10, h // 3) if position == 'top' else (2 * h // 3, h - 10)
            _, score = match_pattern(strip, p, search_range)
            if score > 455:
                matched_all = False
                break
        if matched_all:
...
def extract_pattern_candidates(gray, center_x, top_y, bottom_y):
    ...
    for dy in range(-search_margin_y, search_margin_y + 1):
        ...
        if top_y < mid and ty + max_pattern_height < mid:
            pattern_candidates_top.append(strip[ty:ty + max_pattern_height, :].copy())

        if bottom_y > mid and by - max_pattern_height > mid:
            pattern_candidates_bottom.append(strip[by - max_pattern_height:by, :].copy())
...
def process_images():
    ...
    pattern_candidates_top[:] = filter_consistent_patterns(pattern_candidates_top, pattern_capture, 'top')
    pattern_candidates_bottom[:] = filter_consistent_patterns(pattern_candidates_bottom, pattern_capture, 'bottom')
    print(f"✅ Filtered to {len(pattern_candidates_top)} top and {len(pattern_candidates_bottom)} bottom patterns.")
