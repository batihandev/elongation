...
def process_images():
    ...
    precomputed = {}
    for i, frame in enumerate(tqdm(frames, desc="Precomputing grayscale")):
    ...
    for idx, frame in enumerate(tqdm(frames[:pattern_capture_frames], desc="Collecting candidates")):
    ...
    total_pairs = len(pattern_candidates_top) * len(pattern_candidates_bottom)
    print(f"📊 Evaluating {total_pairs} candidate pattern pairs...")

    for pidx, pt in enumerate(tqdm(pattern_candidates_top, desc="Top pattern loop")):
        for pbidx, pb in enumerate(pattern_candidates_bottom):
            total_score = 0
            valid_matches = 0
            temp_results = {}
            if pbidx % 50 == 0:
                tqdm.write(f"Top {pidx+1}/{len(pattern_candidates_top)} — Bottom {pbidx+1}/{len(pattern_candidates_bottom)}")
            ...
            if valid_matches > 0 and total_score < lowest_total_score:
                best_pattern = (pt, pb)
                lowest_total_score = total_score
                match_results.clear()
                match_results.update(temp_results)
    ...
    print("✅ Global best pattern selected with score:", lowest_total_score)

    data = []
    print("🖼 Drawing overlays and saving output frames...")
    for idx, frame in enumerate(tqdm(frames, desc="Drawing pass")):
