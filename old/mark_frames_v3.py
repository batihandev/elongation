...
def detect_markers(gray, center_x, transition_threshold=25):
    h, w = gray.shape
    ...
    def scan(direction='top'):
        ...
        for y in range_y:
            ...
            if patch.shape != (5, 5):
                continue
            mode_val = np.bincount(patch.flatten()).argmax()
            changed = np.sum(np.abs(patch - mode_val) > transition_threshold)
            if changed >= 15:
                return y
        return None
...
def process_images():
    ...
    for i, frame in enumerate(frames):
        ...
        x2 = min(center_x + strip_width + 1, gray.shape[1])
        rebar_strip = gray[:, x1:x2]

        # Optional: enhance contrast or normalize
        rebar_strip = cv2.equalizeHist(rebar_strip)  # contrast boost

        # Then feed into patch-based marker detection
        top_y, bottom_y = detect_markers(rebar_strip, strip_width)
