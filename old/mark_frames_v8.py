...
def process_images():
    ...
    pattern_capture = []
    for i, frame in enumerate(frames[:pattern_capture_frames]):
    ...
    for pt in pattern_candidates_top:
        for pb in pattern_candidates_bottom:
            total_score = 0
            valid_matches = 0
            for frame in frames:
                img_path = os.path.join(input_folder, frame)
    ...
    print("✅ Global best pattern selected.")

    # Match pass: record top/bottom positions
    for frame in frames:
        img_path = os.path.join(input_folder, frame)
        img = cv2.imread(img_path)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        ...
        top_y, _ = match_pattern(strip, selected_top, (10, gray.shape[0] // 3))
        bottom_y, _ = match_pattern(strip, selected_bottom, (2 * gray.shape[0] // 3, gray.shape[0] - 10))
    ...
    for frame in frames:
        if frame not in match_results:
            continue
        top_y, bottom_y = match_results[frame]
        img_path = os.path.join(input_folder, frame)
        img = cv2.imread(img_path)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        center_x = detect_rebar_center_x(gray)
